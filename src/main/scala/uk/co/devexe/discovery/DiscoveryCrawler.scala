...
class DiscoveryCrawler extends WebCrawler {
  ...
  private val LOG = LoggerFactory.getLogger(classOf[CrawlManager])
  private val IMAGE_EXTENSIONS = Pattern.compile(".*\\.(bmp|gif|jpg|png)$")
  private val BROWSE_PATTERN = Pattern.compile("https://.*\\.*/browse/r/h/.*")
  ...
  override def shouldVisit(referringPage: Page, url: WebURL): Boolean = {
    ...
    if(IMAGE_EXTENSIONS.matcher(href).matches()) {
      ...
      href.startsWith("https://discovery.nationalarchives.gov.uk/")
    }
  }
  ...
  private def process(parseData: ParseData): Unit = parseData match {
